...
next.addEventListener('click',()=>{
  const visibleItems = Math.floor(langages.parentElement.offsetWidth / item_langages[0].offsetWidth);
    if(indexCar < item_langages.length - visibleItems){
        indexCar++;
        const width = item_langages[0].offsetWidth + 10; // + gap entre les items
        langages.style.transform = `translateX(-${indexCar * width}px)`;
    }
})
